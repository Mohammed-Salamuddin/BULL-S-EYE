...
/*to display bitmap char(that is display text on the screen)*/
void bitmap_output(int x,int y,char *string,void *font)
{
	int len,i;
	//This function does not return a value,defines the starting point of the the text to draw
	//2f means 2D drawing
	glRasterPos2f(x,y);
	...
	for(i=0;i<len;i++)
	{
		...
		/*glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_10, *c);
		 font type and the character to be displayed. The font type can be one of the following fonts
		GLUT_BITMAP_8_BY_13
		GLUT_BITMAP_9_BY_15
...
void counting()
{
	sprintf(str,"No of arrows:%d",count); //output is stored in str
	bitmap_output(400,40,str,GLUT_BITMAP_HELVETICA_18);
	//above function displays from (400,40) the string in str
...
void disp()
{
	...
	glLoadIdentity();
	//used to set "some matrix which i currently dont know" to identity matrix
	glColor3f(1,1,0);  
	//yellow color
	bitmap_output(0,450,"BLOCK - SHOOTING",GLUT_BITMAP_TIMES_ROMAN_24);
	counting();
...
void draw_target()
{
	if(count1==0)
	{
		...
		glPointSize(30);  		
		glBegin(GL_POINTS);
		glVertex2d(300,450);  //Magenta square of size 30
		glEnd();
	...
	else
	{
		...
		glPointSize(20);
		glBegin(GL_POINTS);
		glVertex2d(300,450);  //white square of size 20
		glEnd();
	}
	if(count2==0)
	{
...
int main(int argc,char **argv)
{
	...
	glutCreateWindow("Bulls Eye Game");
